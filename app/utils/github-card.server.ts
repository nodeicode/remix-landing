...
export function renderEngineeringCard(data: EngineeringCardData, theme: "dark" | "light" = "dark"): string {
	...
	const hasLatency = monitor?.processing?.p99Ns != null;
	const telemetryHealthy = monitor?.heartbeatAgeMs != null && monitor.heartbeatAgeMs <= 90_000;
	const health = telemetryHealthy ? "HEALTHY" : monitor ? "STALE" : "PENDING";
	const healthColor = telemetryHealthy ? colors.good : monitor ? colors.warning : colors.muted;
	...
	const latencyWidth = hasLatency ? Math.max(40, Math.min(220, 220 - Math.log10((monitor?.processing?.p99Ns ?? 1) / 1e3) * 31)) : 0;
...
<text x="38" y="313" fill="${colors.muted}" font-size="11" letter-spacing="1.5">LIVE SYSTEM DIAGNOSTIC · LAST 24 HOURS</text><rect x="38" y="330" width="824" height="105" rx="10" fill="${colors.panelAlt}" stroke="${colors.border}"/>
<circle cx="63" cy="358" r="5" fill="${healthColor}"/><text x="78" y="362" fill="${colors.text}" font-size="12" font-weight="700">${health}</text><text x="78" y="383" fill="${colors.muted}" font-size="11">${monitor?.heartbeatAgeMs != null ? `heartbeat ${Math.max(0, Math.round(monitor.heartbeatAgeMs / 1_000))}s ago` : "telemetry appears when the public monitor has samples"}</text>
${hasLatency ? `<text x="311" y="362" fill="${colors.text}" font-size="13" font-weight="700">p99 ${latency}</text><text x="311" y="383" fill="${colors.muted}" font-size="11">input event → decision</text><rect x="311" y="399" width="220" height="5" rx="2.5" fill="${colors.border}"/><rect x="311" y="399" width="${latencyWidth}" height="5" rx="2.5" fill="${colors.accent}"/>` : `<text x="311" y="362" fill="${colors.text}" font-size="13" font-weight="700">No latency claim published</text><text x="311" y="383" fill="${colors.muted}" font-size="11">sample data is never substituted for real telemetry</text>`}
${monitor ? `<text x="586" y="362" fill="${colors.text}" font-size="13" font-weight="700">${monitor.eventCount.toLocaleString()} events</text><text x="586" y="383" fill="${colors.muted}" font-size="11">observed in the current window</text>` : ""}
